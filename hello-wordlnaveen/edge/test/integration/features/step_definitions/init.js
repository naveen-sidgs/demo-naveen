...
var environment = config['sidgs-${teamName}-hello-wordlnaveen'].env;
...
console.log('&$sidgs-${teamName}-hello-wordlnaveen api: [' +  domains[environment] + ', ' + config['sidgs-${teamName}-hello-wordlnaveen'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['sidgs-${teamName}-hello-wordlnaveen'].app, config['sidgs-${teamName}-hello-wordlnaveen'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['sidgs-${teamName}-hello-wordlnaveen'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
